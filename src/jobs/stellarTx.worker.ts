...
async function processStellarTx(job: Job<StellarTxJobData>): Promise<void> {
  const { txEnvelopeXdr, userId, paymentId } = job.data;
  ...
  const result = await stellarService.submitTransaction(txEnvelopeXdr);
  ...
  if (paymentId) {
    await pool.query(
      "UPDATE payments SET status = 'completed', transaction_hash = $1, updated_at = NOW() WHERE id = $2",
