...
describe("VerificationService", () => {
    ...
    it("creates a verification record and sends confirmation email", async () => {
      ...
      mockQuery
        ...
        .mockResolvedValueOnce({
          rows: [{ email: "mentor@example.com", first_name: "Alice" }],
        }); // email lookup
      ...
      expect(mockEnqueueEmail.mock.calls[0][0].to).toContain(
        "mentor@example.com",
      );
    });
  ...
  describe("reject", () => {
    it("rejects verification and sends email with reason", async () => {
      const verification = makeVerification();
      const rejected = makeVerification({
        status: "rejected",
  ...
  describe("requestMoreInfo", () => {
    ...
    it("throws if verification is not pending", async () => {
      ...
      await expect(
        VerificationService.requestMoreInfo(
          "ver-uuid-1",
          "admin-uuid-1",
          "Need more docs",
        ),
  ...
  describe("list", () => {
    it("uses parameterized $1/$2 for LIMIT/OFFSET without status filter", async () => {
      // Set up mock to return valid data for any query
      mockQuery.mockResolvedValue({ rows: [{ count: "0" }] });

      await VerificationService.list({ page: 1, limit: 10 });
      ...
      // Verify the SQL uses $1 and $2 placeholders (not bare numbers)
      expect(dataCall![0]).toContain("LIMIT $1 OFFSET $2");
      // Verify the values array contains limit and offset
      expect(dataCall![1]).toEqual([10, 0]);
    });

    it("uses parameterized $2/$3 for LIMIT/OFFSET with status filter", async () => {
      mockQuery.mockResolvedValue({ rows: [{ count: "0" }] });

      await VerificationService.list({ status: "pending", page: 1, limit: 10 });

      const dataCall = mockQuery.mock.calls.find(
        (c) => typeof c[0] === "string" && c[0].includes("LIMIT"),
      );
      expect(dataCall).toBeDefined();
      // With status filter, status=$1, LIMIT=$2, OFFSET=$3
      expect(dataCall![0]).toContain("v.status = $1");
      expect(dataCall![0]).toContain("LIMIT $2 OFFSET $3");
      expect(dataCall![1]).toEqual(["pending", 10, 0]);
    });

    it("calculates correct offset for page > 1", async () => {
      mockQuery.mockResolvedValue({ rows: [{ count: "0" }] });

      await VerificationService.list({ page: 3, limit: 10 });
    ...
    it("applies default page=1 and limit=20 when not provided", async () => {
      ...
      await VerificationService.list({});
  ...
  describe("getStatusByMentorId", () => {
    it("returns the latest verification record", async () => {
      ...
      const result =
        await VerificationService.getStatusByMentorId("mentor-uuid-1");
      expect(result?.id).toBe("ver-uuid-1");
    });
