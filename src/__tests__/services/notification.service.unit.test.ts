...
describe('NotificationService', () => {
  beforeEach(() => {
    ...
    mockReset(mockPreferencesModel);
  ...
  describe('create', () => {
    it('should create notification using simplified parameters', async () => {
      ...
      jest.spyOn(NotificationService, 'createNotification').mockResolvedValue(mockNotification as any);
  ...
  describe('sendMeetingUrlNotification', () => {
    it('should send meeting URL notifications to both participants', async () => {
      ...
      jest.spyOn(NotificationService, 'createInAppNotification').mockResolvedValue({} as any);
  ...
  describe('getDefaultTitle', () => {
    it('should return default title for known types', () => {
      expect(NotificationService.getDefaultTitle('booking_confirmed')).toBe('Booking Confirmed');
      expect(NotificationService.getDefaultTitle('payment_processed')).toBe('Payment Processed');
      expect(NotificationService.getDefaultTitle('unknown')).toBe('Notification');
    });
  });

  describe('getDefaultMessage', () => {
    it('should return default message for known types', () => {
      expect(NotificationService.getDefaultMessage('booking_confirmed')).toBe('Your booking has been confirmed.');
      expect(NotificationService.getDefaultMessage('payment_processed')).toBe('Your payment has been processed successfully.');
      expect(NotificationService.getDefaultMessage('unknown')).toBe('You have a new notification.');
    });
  });
  ...
  describe('getNotificationStatus', () => {
    it('should return notification status with delivery history', async () => {
      ...
      mockDeliveryTrackingModel.getLatestStatus.mockResolvedValue(mockLatestStatus as any);

      const result = await NotificationService.getNotificationStatus(notificationId);
  ...
  describe('cancelScheduledNotification', () => {
    it('should cancel scheduled notification', async () => {
      ...
      expect(result).toBe(true);
      expect(mockNotificationsModel.delete).toHaveBeenCalledWith(notificationId);
    });
  });
  ...
  describe('getUnreadNotifications', () => {
    it('should return unread notifications for user', async () => {
      ...
      mockNotificationsModel.getUnreadByUserId.mockResolvedValue(mockNotifications as any);
  ...
  describe('markAsRead', () => {
    it('should mark notification as read', async () => {
      ...
      expect(result).toBe(true);
      expect(mockNotificationsModel.markAsRead).toHaveBeenCalledWith(notificationId);
    });
  });
  ...
  describe('markAllAsRead', () => {
    it('should mark all notifications as read for user', async () => {
      ...
      expect(mockNotificationsModel.markAllAsReadByUserId).toHaveBeenCalledWith(userId);
    });
  });
  ...
  describe('getUserNotifications', () => {
    it('should return user notifications with filters', async () => {
      ...
      mockNotificationsModel.getByUserId.mockResolvedValue(mockNotifications as any);

      const result = await NotificationService.getUserNotifications(userId, options);

      expect(result).toEqual(mockNotifications);
      expect(mockNotificationsModel.getByUserId).toHaveBeenCalledWith(userId, options);
    });
  });
  ...
  describe('validateNotificationRequest', () => {
    ...
    it('should return errors for invalid request', () => {
      ...
      expect(result.errors).toContain('Invalid notification type: invalid_type');
    });
  });
  ...
  describe('cleanupExpiredNotifications', () => {
    ...
    it('should return 0 on error', async () => {
      mockNotificationsModel.deleteExpired.mockRejectedValue(new Error('DB error'));
