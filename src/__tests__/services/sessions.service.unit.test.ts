...
describe('SessionsService (BookingsService)', () => {
  ...
  describe('createBooking', () => {
    ...
    it('should throw error if mentee not found', async () => {
      ...
      mockPool.query.mockResolvedValue({ rows: [{ id: 'mentor-123', role: 'mentor' }] });

      await expect(SessionsService.createBooking(data)).rejects.toThrow('Mentee not found');
    });
    ...
    it('should throw error if mentor not found', async () => {
      ...
      mockPool.query.mockResolvedValue({ rows: [{ id: 'mentee-123', role: 'learner' }] });

      await expect(SessionsService.createBooking(data)).rejects.toThrow('Mentor not found');
    });
    ...
    it('should throw error if user is not a mentor', async () => {
      ...
      await expect(SessionsService.createBooking(data)).rejects.toThrow('User is not a mentor');
    });
    ...
    it('should throw error if mentor has conflict', async () => {
      ...
      await expect(SessionsService.createBooking(data)).rejects.toThrow('Mentor is not available at the requested time');
    });
  });
  ...
  describe('getBookingById', () => {
    ...
    it('should throw error if booking not found', async () => {
      ...
      await expect(SessionsService.getBookingById(bookingId, userId)).rejects.toThrow('Booking not found');
    });
    ...
    it('should throw error if user has no access', async () => {
      ...
      await expect(SessionsService.getBookingById(bookingId, userId)).rejects.toThrow('Access denied');
    });
  });
  ...
  describe('getUserBookings', () => {
    ...
    it('should fetch from database and cache if not cached', async () => {
      ...
      expect(result).toEqual(mockResult);
      expect(mockBookingModel.findByUserId).toHaveBeenCalledWith(userId, undefined);
  ...
  describe('updateBooking', () => {
    it('should update booking successfully', async () => {
      ...
      const mockUpdated = { ...mockBooking, topic: data.topic, notes: data.notes };

      jest.spyOn(SessionsService, 'getBookingById').mockResolvedValue(mockBooking as any);
      mockBookingModel.checkConflict.mockResolvedValue(false);
      mockBookingModel.update.mockResolvedValue(mockUpdated as any);
      mockCacheService.del.mockResolvedValue(true);

      const result = await SessionsService.updateBooking(bookingId, userId, data);
    ...
    it('should throw error if booking status is not updatable', async () => {
      ...
      await expect(SessionsService.updateBooking(bookingId, userId, data)).rejects.toThrow('Cannot update booking in current status');
    });
  });
  ...
  describe('confirmBooking', () => {
    it('should confirm booking successfully', async () => {
      ...
      const mockUpdated = { ...mockBooking, status: 'confirmed', updated_at: new Date() };
    ...
    it('should throw error if not mentor', async () => {
      ...
      await expect(SessionsService.confirmBooking(bookingId, userId)).rejects.toThrow('Only the mentor can confirm bookings');
    });
  });
  ...
  describe('completeBooking', () => {
    it('should complete booking successfully', async () => {
      ...
      const mockUpdated = { ...mockBooking, status: 'completed', updated_at: new Date() };
    ...
    it('should throw error if session not ended', async () => {
      ...
      await expect(SessionsService.completeBooking(bookingId, userId)).rejects.toThrow('Cannot complete booking before session ends');
    });
  });
  ...
  describe('cancelBooking', () => {
    it('should cancel booking successfully', async () => {
      ...
      const mockUpdated = { ...mockBooking, status: 'cancelled', updated_at: new Date() };
      ...
      const result = await SessionsService.cancelBooking(bookingId, userId, reason);
  ...
  describe('rescheduleBooking', () => {
    it('should reschedule booking successfully', async () => {
      ...
      const mockUpdated = { ...mockBooking, scheduled_at: newScheduledAt, status: 'rescheduled' };
      ...
      const result = await SessionsService.rescheduleBooking(bookingId, userId, newScheduledAt);
  ...
  describe('updatePaymentStatus', () => {
    it('should update payment status successfully', async () => {
      ...
      const result = await SessionsService.updatePaymentStatus(bookingId, stellarTxHash, transactionId);
